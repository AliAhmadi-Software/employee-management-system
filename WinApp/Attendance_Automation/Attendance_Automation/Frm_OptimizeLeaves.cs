...
{
    public partial class Frm_OptimizeLeaves : Frm_Template
    {
        Attendance_DBEntities Entities = new Attendance_DBEntities();
        public Frm_OptimizeLeaves()
        {
            InitializeComponent();
        }
        ...
        private void btn_StartOptimize_Click(object sender, EventArgs e)
        {
            ...
            if (dr == DialogResult.Yes)
            {
                ...
                int sqlQuery = Entities.Database.ExecuteSqlCommand("delete from Tbl_Leaves where From_Date>=@p0 AND Until_Date<=@p1", fromDate, untilDate);
