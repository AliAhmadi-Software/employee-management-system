...
{
    public partial class Frm_OptimizeLeavesTime : Frm_Template
    {
        Attendance_DBEntities Entities = new Attendance_DBEntities();
        public Frm_OptimizeLeavesTime()
        {
            InitializeComponent();
        }
        ...
        private void btn_StartOptimize_Click(object sender, EventArgs e)
        {
            ...
            if (dr == DialogResult.Yes)
            {
                ...
                int sqlQuery = Entities.Database.ExecuteSqlCommand("delete from Tbl_LeavesTime where LeaveDate>=@p0 AND LeaveDate<=@p1", fromDate, untilDate);
